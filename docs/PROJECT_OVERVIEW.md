...
### 2.1 核心问题
跑步者（尤其入门与节奏训练人群）难以长期保持稳定步频（Cadence，即每分钟步数）。步频不稳会直接导致配速波动、体力消耗不均与受伤风险上升。

### 2.2 解决方案
RunBeat 是一款**高精度跑步节拍器**：通过听觉（3 种节拍音色 + 小节重音）与视觉（脉冲动画 + 拍位指示）双重反馈，帮助跑者将步频锁定在目标 BPM。与一般节拍器不同，它针对跑步场景做了专门设计：
